...
import { UserInterface, Group, Record } from './../../libs/models'
...
recordsRouter.get('/', (req, res, next) => {
  ...
  if (req['group'] as Group == 'admins' && req.query.scope && req.query.scope == 'admin') {
    ...
    if (req['group'] as Group == 'vendors') {
      Record.find({
        "signatures.personnel": userID
      }, {
        contents: 0
      }).then(records => res.json(records)).catch(next)
    } else { // "securityPersonnel" | "users"
      // Match Object
      let predicates: {[_: string]: string}[] = [
        { "signatures.personnel": userID }
      ]
